...
with open("smclass/c1021/1.html","w",encoding="utf-8") as f:
    f.write(res.text)



# html, css 정보를 가진 소스변경, 권장되는 방법
soup = BeautifulSoup(res.text,"lxml") # str -> html태그,css태그 사용될수 있는 

#BeautifulSoup 사용
#태그 출력, 태그 글자 출력
# print(soup.title)             # title태그
# print(soup.title.get_text())  # title태그 문자 - text, get_text()

# print("없는 태그 : ",soup.titletitle) # 없는 태그입력시 None
# # print("없는 태그 : ",soup.titletitle.get_text()) # 없을시 에러
...
# print(soup.find("h2",{"class":"rankingnews_tit"}).text) #h2태그 class=rankingnews_tit
print(soup.find_all("div")) 
print(len(soup.find_all("div")))
print(type(soup.find_all("div")))

with open("smclass/c1021/2.html","w",encoding="utf-8") as f:
