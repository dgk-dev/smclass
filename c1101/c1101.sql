...
-- 월별로 멤버들을 정리
select substr(mdate, 4,2)||'월' 월, count(name)||'명' 인원수 from member
group by substr(mdate, 4,2) order by substr(mdate, 4,2) asc;

-- employees 테이블에서 부서별 인원을 출력하시오.
select department_id,count(emp_name) from employees
...
-- 안에 있는 데이터가 너무 커 바꿀 수가 없음
alter table emp4
modify(emp_name varchar2(10));

-- 컬럼 타입 변경 -> 컬럼안 데이터가 null이면 가능
-- 다른 타입인 경우 데이터를 null로 변경한 후 타입을 변경해야함.
...
update bmember set pw='2222' where name='강감찬';
...
select employee_id,emp_name,email,salary,department_name
...
update mem2 set id='abc', pw='1111', name='강규석', email='kangmumu@gmail.com'
where id='Trineman';
...
-- bmember 테이블 id, foreign key로 board,board2 등록
-- foreign key : 외래키
